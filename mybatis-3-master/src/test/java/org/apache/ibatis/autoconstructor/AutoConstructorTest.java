...
class AutoConstructorTest {
  private static Logger logger = LoggerFactory.getLogger(AutoConstructorTest.class);
  private static SqlSessionFactory sqlSessionFactory;

  @BeforeAll
  static void setUp() throws Exception {
    // create a SqlSessionFactory
    // 创建SqlSessionFactory对象，基于mybatis-config.xml文件
    try (Reader reader = Resources.getResourceAsReader("org/apache/ibatis/autoconstructor/mybatis-config.xml")) {
      sqlSessionFactory = new SqlSessionFactoryBuilder().build(reader);
    }

    // populate in-memory database
    //初始化数据到内存数据库中，基于CreateDB.sql文件
  ...
  //一般情况下，pojo对象里，不适用基本类型。
  @Test
  void primitiveSubjects() {
    try (SqlSession sqlSession = sqlSessionFactory.openSession()) {
      final AutoConstructorMapper mapper = sqlSession.getMapper(AutoConstructorMapper.class);
      logger.info("getSubjectOne::{}",mapper.getSubject(1));
      logger.info("getSubjects::{}",mapper.getSubjects());
